package com.example.oauth2_0example.helper;
...
public class WebHelper {
	...
	/**
	 * 파라미터를 전달받아서 Float로 형변환 하여 리턴한다.
	 * 
	 * @param fieldName
	 *            - 파라미터 이름
	 * @param defaultValue
	...
	public int[] getIntArray(String fieldName, int[] defaultValue) {
		...
		if (param != null) { // 수신된 파라미터가 존재한다면?
			if (param.length > 0) { // 배열의 길이가 0보다 크다면?
				result = Arrays.stream(param).mapToInt(Integer::parseInt).toArray(); // 리턴을 위해서 준비한 변수에 수신한 값을 복사한다.
			}
		}
